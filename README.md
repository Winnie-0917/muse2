...
2. 把 `Data/<編號>.csv`（原始 EEG）**改名搬移**成 `Data/<實驗檔名>_Original.csv`。

第 2 步是搬移不是複製：跑完實驗後 `Data/` **不會留下流水號那份**，
同一段原始 EEG 不會在 `Data/` 裡存兩次。
...
- **`Features/<編號>.csv` 仍是流水號**，而且會被重複使用。流水號是看 `Data/` 現有的
  編號往上加，實驗把流水號檔搬走後編號會重新從 1 算起，所以下一次實驗的
  `Features/1.csv` 會覆蓋掉上一次的。被覆蓋的那份在實驗當下就已經複製進 `Model/` 了，
  所以不會真的掉資料；要留著中繼結果的話，先手動改名再跑下一次實驗。
- 萬一 `Model/` 的歸檔被刪掉、編號重新從 S1 算起而撞名，會自動往後改成
  `_Original_2.csv`、`_Original_3.csv`，不會覆蓋掉上一次實驗的原始資料。
- Features 的歸檔是複製不是搬移，實驗分類選錯時只要刪掉 `Model/` 裡那份重做即可。
